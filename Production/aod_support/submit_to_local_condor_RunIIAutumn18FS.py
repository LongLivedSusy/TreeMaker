...
cmds = []
output_folder = "srm://dcache-se-cms.desy.de/pnfs/desy.de/cms/tier2/store/user/vkutzner/NtupleHub/ProductionRun2v3_SMS3/"
redo_miniaod = True
...
for campaign in campaigns:
    ...
    for inputFilesConfig in campaigns[campaign]["inputFilesConfigs"]:
        ...
        for i, file_name in enumerate(file_names):
            ...
            cmsrun = """cd $CMSSW_BASE/src/TreeMaker/Production/test/;
                      pwd;
                      rm info* *root;
                      echo "First time"; cmsRun runMakeTreeFromMiniAOD_cfg.py scenario=%s inputFilesConfig=%s nstart=%s nfiles=1 outfile=%s numevents=-1;
                      ../aod_support/convert_AOD_to_miniAOD.py --infile=$(cat info_aodfilenames) --outfile=%s_miniaod.root --nev=-1;
                      echo %s_miniaod.root > info_miniaods;
                      echo "Second time"; cmsRun runMakeTreeFromMiniAOD_cfg.py scenario=%s inputFilesConfig=%s nstart=%s nfiles=1 outfile=%s numevents=-1;
                      source /cvmfs/grid.desy.de/etc/profile.d/grid-ui-env.sh; 
                      rootls %s_RA2AnalysisTree.root:TreeMaker2 | grep PreSelection && (eval `scram unsetenv -sh`; gfal-copy %s_RA2AnalysisTree.root %s) || echo "Failed!";
                      rm %s_miniaod.root %s_RA2AnalysisTree.root""" % (campaigns[campaign]["scenario"], this_inputFilesConfig, i, file_name, file_name, file_name, campaigns[campaign]["scenario"], this_inputFilesConfig, i, file_name, file_name, file_name, output_folder, file_name, file_name)
...
for i_bigchunk, bigchunk in enumerate(bigchunks):
    ...
    runParallel(bigchunk, "grid", use_sl6=False, condorDir="condorPMSSM18_%s" % i_bigchunk, use_more_time=21600, confirm=False, tarball=True, recreateTarball=True, cmsbase="/afs/desy.de/user/k/kutznerv/dust/shorttrack/treemaker/CMSSW_10_2_7")
