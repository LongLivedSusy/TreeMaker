...
if custom_args != "":
    ...
    command = 'cmsDriver.py miniAOD-prod -s PAT --processName=%s --eventcontent MINIAOD --runUnscheduled --data --conditions %s --era %s --filein %s --fileout file:%s -n %s' % (outrecolabel, global_tag, era, options.infile, options.outfile, options.nev)
else:
    command = 'cmsDriver.py miniAOD-prod -s PAT --processName=%s --eventcontent MINIAODSIM --runUnscheduled --mc --conditions %s --era %s --filein %s --fileout file:%s -n %s' % (outrecolabel, global_tag, era, options.infile, options.outfile, options.nev)
